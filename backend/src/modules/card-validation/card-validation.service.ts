import {
  ...
  CARD_VALIDATION_ERROR_CODES,
} from "./card-validation.types";
import cardValidator from "card-validator";

export const validateCard = (
  card: CardValidationRequest
): CardValidationResponse => {
  ...
  if (!numberValidation.isValid) {
    return {
      valid: false,
      error: {
        code: CARD_VALIDATION_ERROR_CODES.INVALID_CARD_NUMBER,
  ...
  if (!expirationValidation.month || !expirationValidation.year) {
    return {
      valid: false,
      error: {
        code: CARD_VALIDATION_ERROR_CODES.INVALID_EXPIRATION_DATE,
  ...
  if (!expirationValidation.isValid) {
    return {
      valid: false,
      error: {
        code: CARD_VALIDATION_ERROR_CODES.EXPIRED_CARD,
